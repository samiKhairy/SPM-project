...
namespace recordio
{
    ...
    {
        return *reinterpret_cast<const uint32_t *>(&buffer[offset + sizeof(uint64_t)]);
    }
    ...
    inline size_t appendRecord(std::vector<char> &buffer, uint64_t key, uint32_t len, std::istream &source)
    {
        ...
        std::memcpy(&buffer[start], &key, sizeof(uint64_t));
        std::memcpy(&buffer[start + sizeof(uint64_t)], &len, sizeof(uint32_t));
        source.read(&buffer[start + kHeaderSize], len);
    ...
    inline void writeAt(std::ostream &out, const std::vector<char> &buffer, size_t offset)
    {
        ...
        out.write(&buffer[offset], kHeaderSize + len);
    }
