...
static void dc_divide(const MergeTask &t, std::vector<MergeTask> &children)
{
    ...
    children.push_back(MergeTask{std::move(left), make_temp_path(*t.cfg), true, t.cfg});
    children.push_back(MergeTask{std::move(right), make_temp_path(*t.cfg), true, t.cfg});
}

static bool dc_cond(const MergeTask &t)
{
    return t.inputs.size() <= t.cfg->leaf_k;
}

static void dc_base(const MergeTask &t, MergeTask &result)
{
    result = t;
    if (t.inputs.empty())
...
static void dc_combine(std::vector<MergeTask> &children, MergeTask &result)
{
    if (children.empty())
        return;
    ...
    if (children.front().cfg == nullptr)
        throw std::runtime_error("dc_combine: missing config");
    const DcConfig &cfg = *children.front().cfg;
    if (result.output.empty())
    {
        result.output = make_temp_path(cfg);
        result.owns_output = true;
    }
    result.cfg = &cfg;
    sortutil::merge_k_runs(outputs, result.output, cfg.inbuf, cfg.outbuf, cfg.payload_max);

    for (const auto &c : children)
    {
        if (c.owns_output && fs::absolute(c.output) != fs::absolute(cfg.final_out))
        {
            std::error_code ec;
...
int main(int argc, char **argv)
{
    try
    {
        ...
        MergeTask result{{}, final_out, false, &cfg};

        ff::ff_DC<MergeTask> dc(dc_divide, dc_combine, dc_base, dc_cond, root, result, workers, 0, 0, nullptr);
        if (dc.run_and_wait_end() < 0)
            throw std::runtime_error("Failed to run ff_DC");
